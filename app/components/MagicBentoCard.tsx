...
const DEFAULT_GLOW_COLOR = '16, 185, 129'; // emerald-500 en rgb
...
export const GlobalSpotlight: React.FC<{
  ...
  glowColor?: string;
  sectionClass: string;
}> = ({ gridRef, disableAnimations = false, enabled = true, spotlightRadius = DEFAULT_SPOTLIGHT_RADIUS, glowColor = DEFAULT_GLOW_COLOR, sectionClass }) => {
  ...
  useEffect(() => {
    ...
    const handleMouseMove = (e: MouseEvent) => {
      ...
      const section = gridRef.current.closest(`.${sectionClass}`);
      ...
      const targetOpacity = minDistance <= proximity ? 0.7 : minDistance <= fadeDistance ? ((fadeDistance - minDistance) / (fadeDistance - proximity)) * 0.7 : 0;
      gsap.to(spotlightRef.current, { opacity: targetOpacity, duration: targetOpacity > 0 ? 0.2 : 0.5, ease: 'power2.out' });
    };

    const handleMouseLeave = () => {
      gridRef.current?.querySelectorAll('.pricing-card').forEach(card => (card as HTMLElement).style.setProperty('--glow-intensity', '0'));
  ...
  }, [gridRef, disableAnimations, enabled, spotlightRadius, glowColor, sectionClass]);
